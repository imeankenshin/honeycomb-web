import { type JSX, type Signal, createContext, createSignal, useContext } from "solid-js";
...
const tabContext = createContext<[...Signal<string>, id: string]>();
...
export function TabList(props: TabListProps) {
  const focusableElements = ":is(button, [href], input, select, textarea):not([disabled])";
  return (
    <div
      ...
      onKeyDown={(event) => {
        ...
        const tabs = list.querySelectorAll<HTMLButtonElement>(focusableElements);
        const index = Array.from(tabs).indexOf(event.target as HTMLButtonElement);
